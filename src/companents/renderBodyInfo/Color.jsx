export const colors = (flight, type) => {
    if (flight?.landed === "ПРИ") {
        return type === "border" ? "border-l-4 border-green-500" : "text-green-500";
    }

    if (flight?.tookoff === "ВЫЛ") {
        return type === "border" ? "border-l-4 border-yellow-500" : "text-yellow-500";
    }

    // Если рейс задержан (для вылетающих)
    if (flight?.timedelay && flight.dep && flight?.time !== flight?.timedelay) {
        return type === "border" ? "border-l-4 border-yellow-400" : "text-yellow-400";
    }
